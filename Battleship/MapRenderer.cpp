...
MapRenderer::MapRenderer(std::vector<std::vector<MapGenerator::CellType>> map,const sf::Texture& terrainTexture) : terrainTexture(terrainTexture)
{
	...
	for (int i = 0; i < ARRAY_SIZE; i++) {
		for (int j = 0; j < ARRAY_SIZE; j++) {
			...
			tile[0].texCoords = sf::Vector2f(x*WALL_SIZE +OFFSET_ +2*x, y*WALL_SIZE);
			tile[1].texCoords = sf::Vector2f(x*WALL_SIZE + OFFSET_ + 2 * x, (y + 1)*WALL_SIZE);
			tile[2].texCoords = sf::Vector2f((x + 1)*WALL_SIZE + OFFSET_ + 2 * x, (y + 1)*WALL_SIZE );
			tile[3].texCoords = sf::Vector2f((x + 1)*WALL_SIZE  +OFFSET_ + 2 * x, y*WALL_SIZE);
